...
#[test]
fn fmi_yaml() {
    let parsing_cfg = configs::parsing::Config::from_yaml(resources::NORMALIZED_FMI_YAML);
    assert!(configs::writing::network::graph::Config::try_from_yaml(
        resources::NORMALIZED_FMI_YAML
    )
    .is_err());
    configs::writing::routing::Config::from_yaml(resources::NORMALIZED_FMI_YAML);
    assert!(
        configs::routing::Config::try_from_yaml(resources::NORMALIZED_FMI_YAML, &parsing_cfg)
            .is_err()
    );
}
...
fn fmi_graph() {
    ...
    // means
    let mean_distance = Kilometers(15.313);
    let mean_speed = KilometersPerHour(76.153846154);
    let mean_duration = Hours::from(Seconds(685.098461538));
    ...
    .map(|(idx, src, dst, kilometers, kmph, s)| {
        ...
        TestEdge::new_fwd(
            ...
            src,
            dst,
            Kilometers(Kilometers(kilometers) / mean_distance),
            KilometersPerHour(KilometersPerHour(kmph) / mean_speed),
            Hours(Hours::from(Seconds(s)) / mean_duration),
        )
    })
    .collect();
