...
pub mod id;
use id::{NodeId, EdgeId};
pub mod idx;
use idx::{NodeIdx, EdgeIdx};
...
impl GraphBuilder {
    ...
    pub fn push_node(&mut self, id: Option<NodeId>, lat: f64, lon: f64) -> &mut Self {
    ...
    pub fn finalize(mut self) -> Graph {
        //------------------------------------------------------------------------------------------
        // sort nodes by ascending NodeId

        self.nodes.sort_by(|n0, n1| n0.id.cmp(&n1.id));

        //------------------------------------------------------------------------------------------
        // sort edges by ascending src-id, then by ascending dst-id
        ...
        let mut i = 0;
        let mut offset = 0;
        graph.offsets.push(EdgeIdx(offset));
...
impl Graph {
    ...
    pub fn with_capacity(node_capacity: usize, edge_capacity: usize) -> Graph {
        // offsets -> n+1 due to method `leaving_edges(...)`
        Graph {
            nodes: Vec::with_capacity(node_capacity),
            edges: Vec::with_capacity(edge_capacity),
    ...
    pub fn node_id_from(&self, idx: NodeIdx) -> NodeId {
        self.nodes[idx].id
    }

    pub fn node_idx_from(&self, id: NodeId) -> Result<NodeIdx, NodeIdx> {
    ...
    pub fn edge(&self, src_idx: NodeIdx, dst_idx: NodeIdx) -> &Edge {
    ...
    pub fn leaving_edges(&self, node_idx: NodeIdx) -> &[Edge] {
